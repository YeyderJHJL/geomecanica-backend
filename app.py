"""
Servidor Flask — Mapeo de Ventanas Geomecánicas
Levanta el HTML corregido y expone:
  POST /generar_json      → convierte formulario al JSON objetivo
  POST /insertar_ventana  → ejecuta sp_insertar_ventana_completa en SQL Server
"""
...
def get_connection():
    ...
    server   = os.environ["DB_SERVER"]      # ej. "localhost" o "192.168.1.10,1433"
    database = os.environ["DB_DATABASE"]    # ej. "GeomecDB"
    username = os.environ.get("DB_USER")    # None → usa autenticación Windows
    ...
    if username:
        ...
        # Autenticación Windows integrada (sin usuario/contraseña)
        conn_str = (
            f"DRIVER={{{driver}}};"
            f"SERVER={server};"
            f"DATABASE={database};"
...
# ─────────────────────────────────────────────────────────────────────────────
# TABLAS RMR (idénticas al Excel — hoja RMR)
# ─────────────────────────────────────────────────────────────────────────────
...
# Relleno: tipo 1 = duro (si, sf, ep, ox), tipo 2 = blando (g, cl, ca), tipo 3 = sin relleno (cwf)
RELLENO_TABLA = {
...
def resistencia_from_ucs(ucs):
    for ucs_min, ucs_max, codigo, r89, r76 in RESISTENCIA_TABLA:
        if ucs >= ucs_min and ucs < ucs_max:
...
def calc_jv_rqd(rows):
    ...
    jv = 0.0
    for fam, espaciamientos in familias.items():
...
def calc_espac_prom(rows):
    """Promedio ponderado de espaciamiento (como hace el HTML: Σ(nstr*espac)/Σnstr)."""
    num = 0.0
    den = 0.0
    for r in rows:
...
def calc_row_scores(row):
    """Calcula los ratings de condición de discontinuidad para una fila."""
...
# ─────────────────────────────────────────────────────────────────────────────
# GENERACIÓN DEL JSON OBJETIVO
# ─────────────────────────────────────────────────────────────────────────────

def generar_json_objetivo(data):
    """
    Recibe el payload del formulario HTML y devuelve el JSON en el formato objetivo.
    data = {
        header: { ... campos del encabezado ... },
        rows:   [ ... filas de discontinuidades ... ],
        rmr:    { agua_codigo, ucs_mpa, is50_mpa, gsi_estructura, gsi_superficie,
                  gsi_visual, control_estructural, efectos_voladura, comentario }
    }
    """
    hdr = data.get("header", {})
    rows = data.get("rows", [])
    ...
    # Condición discontinuidades (promedio ponderado por nstr)
    num89 = num76 = den = 0.0
    ...
    # — Fecha —
    fecha_raw = hdr.get("fecha", "")
    if not fecha_raw:
        fecha_raw = datetime.today().strftime("%Y-%m-%d")
    ...
    resultado = {
        ...
        "campania":             campania,
        "este_ini":             _num(hdr.get("iniX")),
        "norte_ini":            _num(hdr.get("iniY")),
        "cota_ini":             _num(hdr.get("iniC")),
        "este_fin":             _num(hdr.get("finX")),
        "norte_fin":            _num(hdr.get("finY")),
        "cota_fin":             _num(hdr.get("finC")),
        "altura_m":             _num(hdr.get("altura")),
        "dip_talud":            _int(hdr.get("dipT")),
        "lito_1":               hdr.get("lito1", ""),
        "lito_2":               hdr.get("lito2", ""),
        "lito_3":               hdr.get("lito3", ""),
        "unidad_litologica":    hdr.get("lmtM", ""),
        "alteracion_codigo":    hdr.get("altZona", ""),
        "intemperismo_codigo":  hdr.get("intemp", ""),
        "sector":               hdr.get("sector", ""),
        "nivel":                _int(hdr.get("nivel")),
        "sector_geotecnico":    hdr.get("sectG", ""),
...
def _tipo(code):
    """Normaliza código de tipo de estructura al formato del JSON objetivo (J, F, etc.)"""
...
HTML_PATH = Path(__file__).parent / "mapeo_ventana_app.html"


@app.route("/")
def index():
    return send_from_directory(str(Path(__file__).parent), "mapeo_ventana_app.html")


@app.route("/generar_json", methods=["POST"])
def api_generar_json():
    """
    Recibe { header, rows, rmr } y devuelve el JSON objetivo.
    """
    try:
...
# ─────────────────────────────────────────────────────────────────────────────
# ENDPOINT: INSERTAR EN SQL SERVER VÍA STORED PROCEDURE
# ─────────────────────────────────────────────────────────────────────────────

@app.route("/insertar_ventana", methods=["POST"])
def api_insertar_ventana():
    """
    1. Recibe { header, rows, rmr, auto_clusterizar? } igual que /generar_json.
    2. Genera el JSON objetivo.
    3. Llama a dbo.sp_insertar_ventana_completa con ese JSON.
    4. Devuelve { ok, ventana_id, codigo, json_enviado } o { ok, error }.
    """
    try:
        data = request.get_json(force=True)
        auto_clusterizar = int(data.get("auto_clusterizar", 0))

        # — Paso 1: generar JSON objetivo —
        resultado = generar_json_objetivo(data)

        # Copia limpia sin el bloque _calc (es info interna del servidor)
        ...
        try:
            cursor = conn.cursor()
            # Declaramos la variable OUTPUT y leemos el resultado con SELECT
            cursor.execute(
                """
                DECLARE @id INT;
        ...
        return jsonify({
            ...
            "codigo":       resultado_sp.get("codigo"),
            "json_enviado": resultado_sp,
        })
...
@app.route("/health")
def health_db():
    """Verifica estado del servidor y conectividad a la BD."""
    db_ok  = False
    db_msg = ""
    try:
        conn = get_connection()
